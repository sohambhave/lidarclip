...
class LidarClip(pl.LightningModule):
    ...
    def contrastive_loss(self, image_features, lidar_features):
        ...
        logits = logit_scale * image_features.float() @ lidar_features.t().float()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            image_features = self.clip.encode_image(image)
        lidar_features, _ = self.lidar_encoder(point_cloud)
        loss = self.contrastive_loss((image_features), (lidar_features))
...
def train(
    ...
    trainer = pl.Trainer(
        ...
        # callbacks=[checkpoint_callback, learningrate_callback, EarlyStopping(monitor="val_loss", mode="min")],
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=("cosine", "mse", "contrastive"),
    )
